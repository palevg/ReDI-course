...
export default function RecipePage() {
  const { recipesList, getRecipes } = useContext(DataContext);
  ...
  const recipeItem = [
    ...
    { name: "ABV", value: recipe.abv + "%", header: "Alcohol by volume. ", hint: "In short, ABV stands for alcohol by volume. The number represents the total volume of liquid in a beer that is made up of alcohol. So, the higher the ABV, the more alcoholic the drink." },
    { name: "IBU", value: recipe.ibu, header: "International Bitterness Units", hint: " - is a less discernable abbreviation that is as, if not more, important when it comes to selecting the type of beer that is right for you.  The IBU scale measures the bitterness levels in beer (based on the amount of hops added) and helps beer drinkers determine which styles of brews are ideal for their taste buds.  This scale can be tricky, however, because higher IBU levels do not always equate to bitterer flavor." },
    { name: "EBC", value: recipe.ebc, header: "EBC stands for European Brewery Convention", hint: " and it's used as a measure of colour intensity. The higher the EBC, the darker the beer." },
    { name: "SRM", value: recipe.srm, header: "Standard Reference Method. ", hint: "SRM is the method for color assessment of wort or beer as published in the recommended methods of the American Society of Brewing Chemists." },
    { name: "pH", value: recipe.ph, header: "Ph stands for “power of hydrogen” or “potential of hydrogen”. ", hint: "It is the chemical variable that denotes a solution’s acidity or alkalinity. The pH value of a solution indicates its concentration of hydrogen ions. As the concentration of hydrogen ions in a solution decreases, it becomes more alkaline (caustic), that is, its pH value increases. Conversely, as the level of hydrogen ions increases, the solution becomes more acid (corrosive), that is, its pH value decreases." },
    { name: "Attenuation", value: recipe.attenuation_level, header: "Attenuation can range between 0.33 and 0.80, but is usually 0.75. ", hint: "Because fermentation produces ethanol, which has a lower density than water (gravity of 0.787 at 25°C), the apparent attenuation overestimates the actual percentage of sugars consumed." }
  ];
  ...
  const reloadRecipes = async () => {
    try {
      ...
      const filteredRecipe = data.filter(item => item.id === +params.id);
      return filteredRecipe.length > 0 ? setRecipe(filteredRecipe[0]) : setRecipe({});
    } catch (err) {
  ...
  useEffect(() => {
    if (recipesList.length === 0) {
      ...
      const filteredRecipe = recipesList.filter(item => item.id === +params.id);
      filteredRecipe.length > 0 ? setRecipe(filteredRecipe[0]) : setRecipe({});
    }
  }, [params.id]);
